...
unit_of_work(CA, State, more(NewState, WellbeingDeltas)) :-
    ...
    acc_state(State, experiences, Experience, NewState),
...
updated_experience(CA, State, count, PriorExperience, UpdatedExperience) :-
    % The ids of observations from prior timeframe from which the prior experience's origin object was synthesized
...
% Count the current observations that would extend/reverse/end the prior experienced trend
updated_experience(CA, State, trend, PriorExperience, UpdatedExperience) :-
    count_from_prior_object(State, PriorExperience.origin, Count, CountedObservations),
     (Count == 0 ->
        UpdatedExperience = PriorExperience.put([value=ended, confidence=1.0]) 
        ;
        trend_experience(CA, PriorExperience, CountedObservations, Count, UpdatedExperience)
    ).
...
trend_experience(CA, PriorExperience, CountedObservations, Count, Experience) :-
    average_confidence(CountedObservations, CurrentConfidence),
    CurrentConfidence > 0,
    PriorObservationIds = PriorExperience.origin.support,
    count(PriorObservationIds, PriorCount),
    % PriorObservationIds are already sorted
    sorted_ids(CountedObservations, CountedObservationIds),
    append(PriorObservationIds, CountedObservationIds, ObservationIds),
    synthetic_object(ObservationIds, Object),
    (Count == PriorCount ->
        TrendExperience = experience{object:Object, kind:trend, value:same, y:CA}
    ;
    Count > PriorCount ->
    ...
    trend_value(PriorExperience, PriorTrendValue),
    trend_confidence(PriorTrendValue, PriorExperience.confidence, CurrentConfidence, TrendExperience.value, Confidence),
    Experience = TrendExperience.put(confidence, Confidence).

trend_value(TrendExperience, TrendValue) :-
    experience{kind:trend, value:TrendValue} :< TrendExperience, !.
% The trend value of an experience that is not a trend experience is none
trend_value(_, none).

% Confidence in a continuing trend grows 
trend_confidence(TrendValue, PriorConfidence, CurrentConfidence, TrendValue, Confidence) :-
    AverageConfidence is (PriorConfidence + CurrentConfidence) / 2,
    Confidence is min(1.0, AverageConfidence + ((1.0 - AverageConfidence) / 10)).
% COnfidence in a new trend is that of the prior experience from which a trend starts
trend_confidence(PriorTrendValue, _, Confidence, TrendValue, Confidence) :-
    PriorTrendValue \= TrendValue.
...
% A novel trend experience is based on a prior count experience. Count and more experiences are de novo.
new_experience(CA, State, trend, _, Experience) :-
    [Timeframe | _] = State.timeframes,
    predsort(priority, Timeframe.experiences, PriorExperiences),
    member(PriorCountExperience, PriorExperiences),
    PriorCountExperience.kind == count,
    PriorObject = PriorCountExperience.origin,
    count_from_prior_object(State, PriorObject, Count, CountedObservations),
    trend_experience(CA, PriorCountExperience, CountedObservations, Count, Experience).
...
elevated_static_ca_observations(State, Experiences) :-
    findall(Observation, 
        ...
        Observations),
    bagof(Experience, (member(Observation, Observations), observation_as_experience(Observation, Experience)), Experiences).
