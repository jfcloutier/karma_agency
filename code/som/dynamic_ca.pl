...
Data:
    ...
    * parents - parent CAs
    * umwelt - child CAs
	* umwelt_commands - commands available from the umwelt (empy except for level 1 CAs)
	...
	* intent - self-assigned goal - none | goal{id: ID, target: Target, impact: Impact, priority: Priority, intent_level: Level, timeframe_index: Index}
    ...
    * timeframes - [Timeframe, ...] - Contiguous timeframes, latest first. A Timeframe carries over from the previous timeframe its observations, experiences, feeling, intent, plans and wellbeing. Old timeframes can be dropped.
    * age - the number of timeframes the CA since it was created (the CA starts life at age 1)
*/
...
initial_state(Level, Latency, Umwelt, Settings, Wellbeing, State) :-
	...
	umwelt_commands(Umwelt, UmweltCommands),
	put_state(EmptyState, 
		[settings - Settings, alive - true, level - Level, latency - Latency, parents - [],
		 umwelt - Umwelt, umwelt_commands - UmweltCommands,
		 ...
		 timeframes - [], age - 1, feeling - none], State).

% Find all actions supported by effector CAs in the CA's umwelt, if any
umwelt_commands(Umwelt, Commands) :-
	findall(EffectorCA, (member(UmweltCA, Umwelt), is_effector(UmweltCA)), EffectorCAs),
	findall(Commands, (member(EffectorCA, EffectorCAs), effector_commands(EffectorCA, Commands)), AllCommands),
    flatten(AllCommands, Commands),
    log(info, dynamic_ca, "~@ has umwelt commands: ~p", [self, Commands]).

effector_commands(EffectorCA, Commands) :-
	query_answered(EffectorCA, action_domain, ActionDomain),
	findall(Command, (member(Action, ActionDomain), Command = command{effector_ca:EffectorCA, action:Action}), Commands).
...
handled(message(phase_done(Phase, StateDeltas, WellbeingDelta), _), State, NewState) :-
    ...
    (timeframe_continues(State1) ->
		...
		log(info, dynamic_ca, "~@ ended timeframe ~w", [self, Age]),
		level(Level),
		published(end_of_timeframe, [level(Level), count(Age)]),
...
handled(message(prediction(Prediction), _), State, NewState) :-
	TimeStampedPrediction = Prediction.put(when_received, State.age),
...
inc_timeframe_count(State, NewState) :-
	...
	put_state(State, age, Inc, NewState).
