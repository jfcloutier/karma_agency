...
% Intent maybe created, plans maybe found, goal activation experiences may be had, activation predictions may be made
phase_consumes_produces(act, [], [predictions_out, experiences, plans, intent]).
...
transition_states(CA, State, PhaseState, NewCAState) :-
    ...
    log(info, phase, "Phase transition of CA ~w from ~w to ~w with intial ~p (~w)", [CA, EndedPhase, Phase, PhaseState, Age]).
...
work(CA, State, Phase) :-
    ...
    run_phase(CA, Phase, Age, WorkEngine).
    
run_phase(CA, Phase, Age, WorkEngine) :-
    log(info, phase, "(~w) Running phase ~w (~w)", [CA, Phase, Age]),
    work_engine_cranked(WorkEngine, WorkStatus),
    log(info, phase, "(~w) Unit of work in phase ~w completed with status ~p (~w)", [CA, Phase, WorkStatus, Age]),
    work_status_handled(WorkStatus, WorkEngine, CA, Phase, Age).

% Get the next solution to the engine's goal
work_engine_cranked(WorkEngine, WorkStatus) :-
...
work_status_handled(done(StateDeltas, WellbeingDelta), WorkEngine, CA, Phase, Age) :-
    phase_done(CA, Phase, Age, StateDeltas, WellbeingDelta, WorkEngine).

work_status_handled(more(StateDeltas, WellbeingDelta), WorkEngine, CA, Phase, Age) :-
    (phase_timeout(Phase) -> 
        phase_done(CA, Phase, Age, StateDeltas, WellbeingDelta, WorkEngine)
        ;
...
phase_done(CA, Phase, Age, StateDeltas, WellbeingDelta, WorkEngine) :-
    ...
    log(info, phase, "Phase ~w for CA ~p done with deltas ~p and ~p (~w)", [Phase, CA, StateDeltas, WellbeingDelta, Age]),
