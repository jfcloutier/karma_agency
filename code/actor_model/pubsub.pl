...
subscribed(Subscriber, Topic) :-
    name(Pubsub),
    message_sent(Pubsub, subscribed(Subscriber, Topic)).
...
unsubscribed(Subscriber, Topic) :-
    name(Pubsub),
    message_sent(Pubsub, unsubscribed(Subscriber, Topic)).
...
all_unsubscribed(Subscriber) :-
    name(Pubsub),
    message_sent(Pubsub, all_unsubscribed(Subscriber)).

publish(Topic, Payload) :-
    self(Source),
    name(Pubsub),
    message_sent(Pubsub, event(Topic, Payload, Source)).
...
broadcast(event(Topic, Payload, Source), State) :-
    ...
    foreach(member(subscription(Name, Topic), Subscriptions), event_sent_to(event(Topic, Payload, Source), Name)).
