...
singleton_thread_name(Module, Name) :-
	...
	name(Name)], 
		...
		call(ModuleGoal), Exception, 
		(
			log(debug, supervisor, 'Failed to get the singleton thread name of ~w: ~p', [Module, Exception]), fail)).
...
starting_dynamic_child(Supervisor, Kind, Module, Name, Options) :-
	log(debug, supervisor, 'Starting dynamic supervised ~w ~w named ~w with ~p under ~w', [Kind, Module, Name, Options, Supervisor]), % The default restart strategy is transient

	option(
		restart(Restart), Options, transient), Goal =.. [start, Name, Module, Options, Supervisor], ChildGoal =.. [ : , Kind, Goal], 
	send(Supervisor, 
...
run_supervised(Options, ParentSupervisor) :-
	option(
		restarting(Restarting), Options, false), 
	start_static_children(Options), 
	maybe_restart_dynamic_children(Restarting), 
	delete(Options, 
		restarting(_), RestOptions), 
	catch(
		run(RestOptions), Exit, 
...
start_static_children(Options) :-
	...
	forall(
		(
			member(Child, Children), Child \== pubsub), 
...
process_message(start_child(StaticOrDynamic, Kind, Module, Name, Goal, Restart), _) :-
	...
	forget_child(Kind, Module, Name), 
		...
		do_start_child(StaticOrDynamic, Kind, Module, Name, Goal, Restart), Exception, 
...
process_message(exited(Kind, Module, Name, Exit), _) :-
	...
	log(debug, supervisor, 'Child ~w ~w named ~w exited with ~p. Restart is ~w', [Kind, Module, Name, Exit, Restart]), !, % avoid race condition by letting the exiting thread complete its exit
	
	sleep(0.5), 
	maybe_restart_child(StaticOrDynamic, Supervisor, Kind, Module, Name, Goal, Exit, Restart).

process_message(exited(Kind, Module, Name, Exit), _) :-
	log(debug, supervisor, 'Unsupervised ~w child ~w named ~w (~p)', [Kind, Module, Name, Exit]).
...
process_message(query(children, From), _) :-
	...
	findall(
		child(Kind, Name), 
		child(_, Supervisor, Kind, _, Name, _, _), Children), 
	send(From, 
...
stop_all_children(Supervisor) :-
	...
	forall(
		(
			child(_, Supervisor, Kind, Module, Name, _, _)), 
		do_stop_child(Kind, Module, Name)).

start_static_child(pubsub) :-
	thread_self(Supervisor), 
	log(debug, supervisor, 'Starting static supervised pubsub under ~w', [Supervisor]), 
	singleton_thread_name(pubsub, Name), Goal =.. [start, Name, pubsub, [], Supervisor], ChildGoal =.. [ : , worker, Goal], 
...
start_static_child(worker(Name, Module, Options)) :-
	...
	option(
		restart(Restart), Options, transient), Goal =.. [start, Name, Module, Options, Supervisor], ChildGoal =.. [ : , worker, Goal], 
...
start_static_child(supervisor(Name, Options)) :-
	...
	option(
		restart(Restart), Options, transient), % All supervisors are implemented by the supervisor module
		Goal =.. [start, Name, supervisor, Options, Supervisor], ChildGoal =.. [ : , supervisor, Goal], 
...
maybe_restart_dynamic_children(true) :-
	...
	forall(
		child(dynamic, Supervisor, Kind, Module, Name, Goal, Restart), 
		maybe_restart_child(dynamic, Supervisor, Kind, Module, Name, Goal, 
			exit(normal), Restart)).
...
child_restarting_goal(ModuleGoal, RestartingModuleGoal) :-
	ModuleGoal =.. [ : , Kind, Goal], Goal =.. [start, Name, Module, Options, Supervisor], 
	merge_options(
		[restarting(true)], Options, RestartingOptions), RestartingGoal =.. [start, Name, Module, RestartingOptions, Supervisor], RestartingModuleGoal =.. [ : , Kind, RestartingGoal].

forget_all_children(Supervisor) :-
	log(debug, supervisor, 'Forgetting all children of ~w', [Supervisor]), 
	retractall(
		child(_, Supervisor, _, _, _, _)).
