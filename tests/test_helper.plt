...
test(get_matching_dict_message) :-
    ...
    thread_create(
        (thread_self(Thread),
        thread_send_message(Self, message(prediction(prediction{origin:object{type:sensor, id:sensor1}, kind:distance, value:10, confidence:1.0, by: Thread, for:[Self]}), Thread))
        ), 
        Thread),
